    ...
    try:
        ...
        return {
            ...
            "result": {
                ...
                "initial_balance": float(result.initial_balance), "final_balance": float(result.final_balance),
                "total_return_pct": float(result.total_return_pct), "weekly_return_pct": float(result.weekly_return_pct),
                "total_trades": int(result.total_trades), "wins": int(result.wins), "losses": int(result.losses),
                "win_rate": float(result.win_rate), "profit_factor": float(result.profit_factor),
                "max_drawdown_pct": float(result.max_drawdown_pct), "max_daily_loss_pct": float(result.max_daily_loss_pct),
                "sharpe_ratio": float(result.sharpe_ratio), "ftmo_compliant": bool(result.ftmo_compliant),
                "avg_spread": float(result.avg_spread), "avg_slippage": float(result.avg_slippage),
                "rejected_orders": int(result.rejected_orders), "partial_fills": int(result.partial_fills),
                "requotes": int(result.requotes),
                "max_consecutive_losses": int(result.max_consecutive_losses),
                "avg_risk_reward": float(result.avg_risk_reward),
                "equity_curve": result.equity_curve,
